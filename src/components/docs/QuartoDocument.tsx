...
import { Book, Download, ExternalLink, Loader2 } from "lucide-react";
import { docAssetUrl, type QuartoDoc } from "../../lib/docs";

interface QuartoDocumentProps {
  doc: QuartoDoc;
  /** Optional line shown on the right of the footer strip. */
  footerNote?: string;
}

/**
 * Embeds a Quarto-rendered document in the KNBS "digital edition" frame.
 *
 * The rendered HTML is self-contained, so it is loaded straight into an iframe
 * and keeps its own table of contents, search and collapsible sections. The
 * frame around it supplies the KNBS chrome, a PDF download and a full-screen
 * escape hatch.
 */
export function QuartoDocument({ doc, footerNote }: QuartoDocumentProps) {
  const htmlUrl = docAssetUrl(doc.html);
  const pdfUrl = docAssetUrl(doc.pdf);

  // Tracking which document finished loading (rather than a plain boolean)
  // means the loader reappears by itself when we switch to another document.
  const [loadedUrl, setLoadedUrl] = useState<string | null>(null);
  const isLoading = loadedUrl !== htmlUrl;
  ...
  return (
    <div className="max-w-none mx-auto bg-gradient-to-b from-amber-50 to-orange-50 min-h-screen">
            ...
            <div className="flex items-center gap-3 shrink-0">
              ...
              >
                ...
                <span className="hidden md:inline">
                  {doc.external ? "View PDF" : "Download PDF"}
                </span>
              </a>
              <a
                href={htmlUrl}
      ...
      <div className="relative">
        ...
        {/* Book Shadow Effect */}
        <div className="relative max-w-7xl mx-auto px-2 sm:px-4 py-4 sm:py-6">
          <div
          ...
          >
            ...
            {/* Page Content */}
            <div className="relative ml-3 sm:ml-6 bg-white">
              {isLoading && (
                <div className="absolute inset-0 z-10 flex flex-col items-center justify-center gap-3 bg-white text-knbs-700">
                  <Loader2 className="animate-spin" size={28} />
                  <p className="text-sm font-medium">Loading {doc.title}…</p>
                </div>
              )}
              <iframe
                key={htmlUrl}
                src={htmlUrl}
                onLoad={() => setLoadedUrl(htmlUrl)}
                className="w-full border-0 bg-white h-[70vh] min-h-[520px] lg:h-[calc(100vh-260px)]"
                title={`${doc.title} — digital edition`}
      ...
      {/* E-book Footer */}
      <div className="bg-gradient-to-r from-amber-800 to-orange-900 text-white py-3 px-4 sm:px-8">
